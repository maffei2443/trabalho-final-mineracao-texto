...
class Cleaner(TransformerMixin):
    ...
    NOT_LETTER_SPACE = re.compile(r'[^a-z ]')
    def __init__(self, remove_accents=True, remove_alpha_numeric=True, max_word_lenght=2):
        ...
        # TODO: encontrar um nome decente para essa regex
        self.REGEX = re.compile(r'\b\w{,%s}\b' % self.max_word_lenght)
    def fit(self, X, y=None):
        return self

    def shrink_blank(self, string):
        return re.sub(Cleaner.SPACES, ' ', string)
    ...
    def transform(self, X, *_):
        ...
        for text in X:
            ...
            if self.remove_alpha_numeric:
                text = re.sub(Cleaner.NOT_LETTER_SPACE, r'', text)
            else:
